...
def handle_error_code(code):
    error_codes = {
        ...
        81: "Unable to configure DHCP service.\nTry running program as an administrator?",
        ...
        -2147180508: "Try running program as an administrator?\nIs the 'Network Connections Properties currently open? Try closing it."
    }
    code = code[0]
    if code == 0:
        return
...
def switch_to_robot(mac_address, ip_address, subnet_mask='255.0.0.0'):
    nic = get_network_adapter_obj(mac_address)
    return_code = nic.EnableStatic(IPAddress=[ip_address], SubnetMask=[subnet_mask])
    handle_error_code(return_code)
...
def get_connection_options():
    if sys.platform != 'win32':
        raise NotWindows("Could not enumerate mac addresses. This program is currently windows only")
    ...
    for line in os.popen(command):
        ...
        connection_name, network_adapter, physical_address, transport_name = out.groups()
